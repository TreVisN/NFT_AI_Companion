...
namespace UIToolkitDemo
{
    ...
    public class InventoryScreenController : MonoBehaviour
    {
        ...
        [SerializeField] List<EquipmentSO> m_AvailableGear;
        ...
        List<EquipmentSO> FilterAvailableGear()
        {
            var nftManager = FindObjectOfType<NftManager>();
            
            var tempList = new List<EquipmentSO>();
            foreach (var gear in m_AllGear)
            {
                if (gear.ownedByDefault && !gear.isNft)
                {
                    tempList.Add(gear);
                }
                else if (gear.isNft && (nftManager.GetIsItemOwned(gear.nftItemId) || nftManager.GetIsItemPreOwned(gear.nftItemId)))
                {
                    Debug.Log($"Added {gear.name} to the equipment list because NFT was found.");
                    tempList.Add(gear);
                }
            }
        ...
        {
            m_AvailableGear = FilterAvailableGear();
            m_UnequippedGear = SortGearList(m_AvailableGear);
