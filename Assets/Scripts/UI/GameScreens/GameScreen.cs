...
namespace UIToolkitDemo
{
    ...
    public class GameScreen : MonoBehaviour
    {
        ...
        IEnumerator GameWonRoutine(string nftItemId)
        {
            Time.timeScale = 0.5f;
            yield return new WaitForSeconds(k_DelayWinScreen);
            ...
            // TODO: Mint NFT - no button to continue
            m_WinNextButton.text = "Minting..";
            m_WinNextButton.SetEnabled(false);
            
            MintAchievement(nftItemId);

        }

        async void MintAchievement(string nftItemId)
        {
            // Delay by 100 ms for the visual elements
            await Task.Delay(100);
        
            var nftManager = FindObjectOfType<NftManager>();
            var achievementItem = nftManager.GetItemById(nftItemId);
            
            // Minting can take a while, so we can start the minting process and let it run in the background
            // var txHash = await nftManager.MintItem(achievementItem);
            // nftManager.MintItem(achievementItem);
            await Task.Delay(3000);
            
            nftManager.AddUnlockedItem(achievementItem);
            PostMint("veryNiceTxHash");
        }

        void PostMint(string txHash)
        {
            Debug.Log($"Minted an achievement. TxHash: {txHash}");
            
            // NFT minted - enable continue button
            m_WinNextButton.text = "Continue";
            m_WinNextButton.SetEnabled(true);
        }
        ...
        void OnGameWon(string nftItemId)
        {
            if (m_IsGameOver)
                return;

            m_IsGameOver = true;
            StartCoroutine(GameWonRoutine(nftItemId));
        }
